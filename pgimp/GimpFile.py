...
class GimpFile:
    ...
    def create(self, layer_name: str, layer_content: np.ndarray) -> 'GimpFile':
        ...
        code = textwrap.dedent(
            ...
            add_layer_from_numpy(image, '{6:s}', '{5:s}', image.width, image.height, {4:d}, 0, 100, gimpenums.NORMAL_MODE, True)
            save_xcf(image, '{3:s}')
            """
        ).format(
            width,
    ...
    def create_indexed(self, layer_name: str, layer_content: np.ndarray, colormap: Union[np.ndarray, ColorMap]) -> 'GimpFile':
        ...
        code = textwrap.dedent(
            ...
            add_layer_from_numpy(image, '{5:s}', '{4:s}', image.width, image.height, gimpenums.INDEXED_IMAGE, 0, 100, gimpenums.NORMAL_MODE, True)
            save_xcf(image, '{3:s}')
            """
        ).format(
            colormap,
